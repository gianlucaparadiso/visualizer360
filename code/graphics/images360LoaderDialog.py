...
class Images360LoaderDialog(QtWidgets.QMainWindow):
    closed = QtCore.pyqtSignal()
    multi_images360_signal = QtCore.pyqtSignal(SigStringList)
    two_360folders_signal = QtCore.pyqtSignal(SigStringList)
    single_360folder_signal = QtCore.pyqtSignal(SigString)

    def __init__(self):
        super(Images360LoaderDialog, self).__init__()

        # Logger setup
        self.logger = logging.getLogger("logger")

        self.logger.info("Creating Images360LoaderDialog...")
        self.initUI(QtCore.QSize(600, 300))
        self.logger.info("Finished creating Images360LoaderDialog")

    def initUI(self, size: QtCore.QSize):

        # Window setup
        self.setWindowTitle("360 images loader mode selection")
        ...
        self.multi_images360_loader = MultiImages360Loader()
        self.multi_images360_loader.closed.connect(partial(self.setDisabled, False))
        self.multi_images360_loader.confirmed.connect(partial(self.close))
        self.multi_images360_loader.multi_images360_signal.connect(self.emitFilenamesSignal)

        # Two folders images loader
        self.two_folders_images360_loader = TwoFoldersImages360Loader()
        self.two_folders_images360_loader.closed.connect(partial(self.setDisabled, False))
        self.two_folders_images360_loader.confirmed.connect(partial(self.close))
        self.two_folders_images360_loader.two_folders_images360_signal.connect(self.emitFoldersSignal)
    ...
    def setLeftWidgetArea(self):
        ...
        left_up.clicked.connect(self.showMultiImages360Loader)
    ...
    def setCentreWidgetArea(self):
        ...
        centre_up.clicked.connect(self.showTwoFoldersImages360Loader)

        # Set the bottom area to describe the action with a label
        centre_down = QtWidgets.QLabel("Add multiple images together. It requires the directory containing the "
                                       "360 images and the directory containing the data files.", self)
    ...
    def closeEvent(self, event):
        self.multi_images360_loader.reset()
        self.multi_images360_loader.close()
        self.two_folders_images360_loader.reset()
        self.two_folders_images360_loader.close()
        self.closed.emit()

    def showMultiImages360Loader(self):
        self.multi_images360_loader.show()
        self.multi_images360_loader.activateWindow()
        self.multi_images360_loader.raise_()
        self.setDisabled(True)

    def showTwoFoldersImages360Loader(self):
        self.two_folders_images360_loader.show()
        self.two_folders_images360_loader.activateWindow()
        self.two_folders_images360_loader.raise_()
        self.setDisabled(True)

    def readDirectoryPath(self):
        directory = str(QtWidgets.QFileDialog.getExistingDirectory())
        if directory != "":
            self.single_360folder_signal.emit(SigString(directory))
            self.close()
    ...
    def emitFilenamesSignal(self, files_filenames: SigStringList):
        self.multi_images360_signal.emit(files_filenames)

    @QtCore.pyqtSlot(SigStringList)
    def emitFoldersSignal(self, folders_filepaths: SigStringList):
        self.two_360folders_signal.emit(folders_filepaths)


class MultiImages360Loader(QtWidgets.QMainWindow):
    closed = QtCore.pyqtSignal()
    confirmed = QtCore.pyqtSignal()
    multi_images360_signal = QtCore.pyqtSignal(SigStringList)

    def __init__(self):
        super(MultiImages360Loader, self).__init__()

        # Logger setup
        self.logger = logging.getLogger("logger")

        self.logger.info("Creating MultiImages360Loader...")
        self.initUI(QtCore.QSize(720, 300))
        self.logger.info("Finished creating MultiImages360Loader")

    def initUI(self, size: QtCore.QSize):

        # Setup window
        self.setWindowTitle("Multiple 360 Images Loader")
        self.resize(size.width(), size.height())
    ...
    def addEntry(self):
        ...
        pc_label = QtWidgets.QLabel("360 image filepath:")
    ...
    def confirmSelection(self):
        ...
        if filenames:
            self.multi_images360_signal.emit(SigStringList(filenames))
            self.confirmed.emit()
...
class TwoFoldersImages360Loader(QtWidgets.QMainWindow):
    closed = QtCore.pyqtSignal()
    confirmed = QtCore.pyqtSignal()
    two_folders_images360_signal = QtCore.pyqtSignal(SigStringList)

    def __init__(self):
        super(TwoFoldersImages360Loader, self).__init__()

        # Logger setup
        self.logger = logging.getLogger("logger")

        self.logger.info("Creating TwoFoldersImages360Loader...")
        self.initUI(QtCore.QSize(720, 300))
        self.logger.info("Finished creating TwoFoldersImages360Loader")

    def initUI(self, size: QtCore.QSize):
        self.setWindowTitle("Two Folders 360 image Loader")
        self.resize(size.width(), size.height())
    ...
    def addEntry(self):
        ...
        # Construction of the 360 image folder filepath entry
        pc_label = QtWidgets.QLabel("360 images folder path:")
    ...
    def confirmSelection(self):
        ...
        if filenames:
            self.two_folders_images360_signal.emit(SigStringList(filenames))
            self.confirmed.emit()
...
class ReaderEntry(QtWidgets.QWidget):
    ...
    def readFilePath(self, tag: int):
        ...
        if tag == 0:
            dlg.setNameFilter("360 image files (*.png *.jpeg)")
        else:
